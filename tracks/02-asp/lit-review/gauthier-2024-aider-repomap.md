...
**Aider** — бесплатный AI-помощник для программистов, работает в окне терминала (без красивой графики). У него есть штука под названием **RepoMap**, которая решает важную задачу: в большом проекте могут быть десятки тысяч файлов, а AI-помощник за один раз умеет «прочитать» только небольшой кусок текста. Как выбрать, какие куски ему показать?

Идея Aider: использовать тот же подход, что Google использует для ранжирования сайтов (алгоритм называется **PageRank**). Aider строит карту: какой файл «ссылается» на какие другие через имена функций и переменных. Самые «центральные» файлы (на которые многие ссылаются) — самые важные. Их и показывают AI-помощнику.

Главное, что мы возьмём из этой работы в свой стандарт: **строгий лимит размера ответа**. У Aider программа гарантирует — «ни один запрос не вернёт больше, чем влезает в окошко AI». У GitNexus такого нет. Мы сделаем эту гарантию обязательной частью наших правил.
